...
export function nav({ c, f }) {
  ...
  const links = ['Over mij', 'Werk', 'Contact'];
  ...
  return `
    <section style="min-height:100vh;display:flex;align-items:center;padding:8rem 3rem 4rem;position:relative;overflow:hidden;">
      ...
      <div style="position:relative;max-width:800px;">
        ...
        <h1 style="font-family:${f.h};font-size:clamp(3.5rem,8vw,7rem);line-height:1.05;font-weight:700;letter-spacing:-0.02em;margin-bottom:0.5rem;">
          ${voornaam}
        </h1>
        <h1 style="font-family:${f.h};font-size:clamp(3.5rem,8vw,7rem);line-height:1.05;font-style:italic;color:var(--accent);margin-bottom:2rem;">
          ${achternaam}.
        </h1>
        <a style="display:inline-flex;align-items:center;gap:0.75rem;background:var(--accent);color:#0E0E0E;text-decoration:none;font-weight:500;font-size:0.9rem;padding:1rem 2rem;border-radius:50px;">
          Bekijk mijn werk →
        </a>
      </div>
    </section>`;
  ...
  const cards = PROJECTS.map(p => `
    <div style="background:${c.surface};border:1px solid ${c.border};border-radius:12px;overflow:hidden;">
      ...
      <div style="padding:1.5rem;">
        ...
        <p style="font-size:0.9rem;color:${c.muted};margin-bottom:1.25rem;">${p.desc}</p>
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
          ${p.tags.map(t => `<span style="font-size:0.7rem;background:${c.surface2};border:1px solid ${c.border};border-radius:4px;padding:0.25rem 0.6rem;color:${c.muted};">${t}</span>`).join('')}
        </div>
      </div>
    </div>`).join('');
